...
class AlphaEarthFoundations(nn.Module):
    ...
    def __init__(self,
        ...
        # Choose dimensions based on model_size (paper S2.4, scaled down)
        model_size = (model_size or "small").lower()
        if model_size == "tiny":
            # Much smaller model for CPU / low-memory setups
            d_p, d_t, d_s, num_blocks = 32, 128, 256, 4
        elif model_size == "base":
            # Slightly larger than small
            d_p, d_t, d_s, num_blocks = 96, 384, 768, 8
        else:  # "small" (default) — keep current behaviour
    ...
    def _perturb_inputs(self,
                        ...
                        temporal_masks: Optional[Dict[str, torch.Tensor]] = None) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        """
        ...
        for name, x in source_data.items():
            ...
            if temporal_masks is not None and name in temporal_masks:
                keep_mask = keep_mask & temporal_masks[name].to(x.device).bool()
            ...
            student_masks[name] = keep_mask

        return student_sources, student_ts, student_masks

    def forward(self,
                source_data: Dict[str, torch.Tensor],
                timestamps: Dict[str, torch.Tensor],
                valid_periods: List[Tuple[float, float]],
                temporal_masks: Optional[Dict[str, torch.Tensor]] = None,
                decode_timestamps: Optional[Dict[str, torch.Tensor]] = None,
        ...
        if temporal_masks is not None:
            teacher_masks = [
                temporal_masks[name].to(feats_teacher.device).bool()
                for name in self.input_sources.keys()
                if name in temporal_masks
            ]
            if teacher_masks:
                teacher_mask = torch.stack(teacher_masks, dim=0).any(dim=0)

        # Build student inputs via perturbation, then encode student
        student_srcs, student_ts_dict, student_masks = self._perturb_inputs(source_data, timestamps, temporal_masks=temporal_masks)
        ...
        student_time_mask = None
        if student_masks:
            student_time_mask = torch.stack(
                [student_masks[name].to(feats_student.device).bool() for name in self.input_sources.keys()],
                dim=0,
            ).any(dim=0)
        ...
        mu_t = self.summarizer(feats_teacher, ts, vp, mask=teacher_mask)      # (B, H', W', 64)
        mu_s = self.summarizer(feats_student, ts_student, vp, mask=student_time_mask)  # (B, H', W', 64)
        ...
        # Default decoding timestamp is the middle time of the support for each sample.
        # Training can override this per source so predictions align with the selected
        # reconstruction target frame more closely.
